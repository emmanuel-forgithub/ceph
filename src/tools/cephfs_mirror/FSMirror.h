...
private:
  ...
  struct SnapListener : public InstanceWatcher::Listener {
    ...
    void acquire_directory(std::string_view dir_path, std::string_view peer_uuids) override {
      fs_mirror->handle_acquire_directory(dir_path, peer_uuids);
    }
  ...
  void handle_acquire_directory(std::string_view dir_path, std::string_view peer_uuids);
