...
class FSPolicy:
    ...
    def process_updates(self):
        def acquire_message(dir_path, target_peers):
            msg = {
                'dir_path': dir_path,
                'mode': 'acquire'
            }
            if target_peers is not None:
                msg['target_peers'] = sorted(list(target_peers))
            ...
            for dir_path in self.dir_paths:
                ...
                if action_type == ActionType.NONE:
                    ...
                    update_map[dir_path] = {'version': 1,
                                            ...
                                            'target_peers': lookup_info['target_peers']
                    }
                    ...
                    target_peers = lookup_info.get('target_peers')
                    notifies[dir_path] = (lookup_info['instance_id'], acquire_message(dir_path, target_peers))
    ...
    def add_dir(self, dir_path, target_peers: Optional[Set[str]] = None):
        with self.lock:
            lookup_info = self.policy.lookup(dir_path)
            if lookup_info:
                if lookup_info['purging']:
            ...
            if not schedule:
                return
            update_data = {dir_path: {'version': 1, 'instance_id': '', 'last_shuffled': 0.0}}
            if target_peers is not None:
                # Store the peers as a sorted list for consistent JSON representation.
                # An empty list means "no specific peers", which semantically we've said
                # implies "don't mirror anywhere" if the user explicitly provided an empty list.
                # If target_peers was None, we omit the key, implying "mirror to all".
                update_data[dir_path]['target_peers'] = sorted(list(target_peers)) # <--- This is CORRECT
            # else: if target_peers is None, we intentionally don't add the 'target_peers' key
            #       This allows the C++ daemon to distinguish between "mirror to all" (key absent)
            #       and "mirror to none" (key present with empty list).

            update_map = {dir_path: update_data}
            updated = False
...
class FSSnapshotMirror:
    ...
    def add_dir(self, filesystem, dir_path, target_peers: Optional[Set[str]] = None):
        try:
            with self.lock:
                ...
                fspolicy.add_dir(dir_path, target_peers=target_peers)
                return 0, json.dumps({}), ''
        except MirrorException as me:
            return me.args[0], '', me.args[1]
        except Exception as e:
